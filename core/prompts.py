...
def build_analysis_prompt(
    *,
    ...
    lines += [
        ...
        "## Your tasks",
        "",
        "1. Compare sources: contradictions, convergent findings, and strength of evidence (cite by title, URL, or id).",
        "2. Identify research gaps: under-studied questions or weakly supported claims.",
        "3. State **novel, falsifiable hypotheses** tied to those gaps (not vague wishes).",
        "4. Propose **concrete experiment and study designs** that could test the hypotheses. This is "
        "planning only: describe studies at the level a grant or protocol outline would use.",
        "5. Suggest further reading, methods, or data to seek.",
        "",
        "Separate **evidence-backed** statements from **speculation** and label them clearly.",
        "",
        "## Required structure for your answer",
        "",
        "Use Markdown with **exactly** these level-2 headings, in order:",
        "",
        "## Conflicts",
        ...
        "### Under `## Hypotheses`",
        "",
        "Number each hypothesis (**H1**, **H2**, …). For each: one crisp falsifiable claim; which corpus items "
        "or gaps motivate it; what kind of observation would weaken or refute it.",
        "",
        "### Under `## Experiment proposals`",
        "",
        "For each proposal (reference **H1**, **H2**, … where relevant), include short labeled bullets:",
        "",
        "- **Objective**",
        "- **Design type** (e.g. observational study, randomized comparison, benchmark or simulation study, "
        "human-participant study, field study: choose what fits the domain)",
        "- **Variables** (independent, dependent, key covariates)",
        "- **Metrics and success criteria**",
        "- **Controls or baselines**",
        "- **Data, setting, or cohort needs** (do not assume you have access)",
        "- **Feasibility and ethics** (brief)",
        "- **Main risks and confounders**",
        "",
        "**Do not** include program source code, shell commands, or runnable pipelines. Do not describe "
        "executing experiments in silico here. Only describe what researchers would plan or run in principle.",
        "",
    ]
    return "\n".join(lines)
